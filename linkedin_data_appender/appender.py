...
def enrich_candidate(
    ...
    *,
    ...
    """Enrich a single candidate LinkedIn URL into a structured profile."""
    if html is not None:
        if not url:
        ...
        try:
            ...
            profile.source = "html"
            return profile
        except Exception as exc:  # Keep callers able to handle one bad profile.
            return CandidateProfile(linkedin_url=url, source="error", error=f"parse failed: {exc}")

    return profile_for_url(
        url,
        html_directory=Path(html_directory) if html_directory else None,
        timeout_seconds=timeout_seconds,
        user_agent=user_agent,
    )
...
def profile_to_json(profile: CandidateProfile) -> str:
    return json.dumps(profile.as_dict(), indent=2, ensure_ascii=False)
