...
class AppenderTests(unittest.TestCase):
    ...
    def test_append_linkedin_data_from_html_dir(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            input_path = temp_path / "input.csv"
            output_path = temp_path / "output.csv"
            html_dir = temp_path / "profiles"
            html_dir.mkdir()
            (html_dir / "jane_doe.html").write_text(
                "<html><title>Jane Doe - Designer at Studio | LinkedIn</title></html>",
                encoding="utf-8",
            )
            input_path.write_text(
            ...
            append_linkedin_data(input_path, output_path, html_directory=html_dir)
